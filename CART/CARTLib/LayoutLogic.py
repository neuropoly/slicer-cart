...
class CaseIteratorLayoutLogic(ScriptedLoadableModuleLogic):
    ...
    def assignLayoutDescription(self, layoutDescription):
        ...
        if layoutNode.IsLayoutDescription(layoutNode.SlicerLayoutUserView):
            layoutNode.SetLayoutDescription(
                layoutNode.SlicerLayoutUserView, layoutDescription
            )
        else:
    ...
    def viewerPerVolume(
        ...
        layoutDescription += self.sliceViewItemPattern.format(
            viewName=viewName, orientation=default_orientation, color=color
        )
        ...
        if rows > 0:
            ...
            for row in range(int(rows)):
                ...
                for column in range(int(columns)):
                    ...
                    rgb = [
                        int(round(v * 255))
                        for v in self.lookupTable.GetTableValue(index)[:-1]
                    ]
                    color = "#%0.2X%0.2X%0.2X" % tuple(rgb)
                    layoutDescription += self.sliceViewItemPattern.format(
                        viewName=viewName, orientation=default_orientation, color=color
        ...
        for index in range(len(actualViewNames)):
            ...
            if volumeNodeID != "" and volume_orientation is not None:
                ...
                if volume_orientation not in orientation_reference_volumes:
                    orientation_reference_volumes[volume_orientation] = volumeNodes[
                        index
                    ]
        ...
        for volume_orientation in orientation_reference_volumes:
            self.rotateToVolumePlanes(
                orientation_reference_volumes[volume_orientation],
                orientation_sliceNodes[volume_orientation],
            )
        self.snapToIJK()
    ...
    def viewersPerVolume(
        self, volumeNodes=None, background=None, label=None, include3D=False
    ):
        ...
        for volumeNode in volumeNodes:
            ...
            for orientation in orientations:
                ...
                color = "#%0.2X%0.2X%0.2X" % tuple(rgb)
                layoutDescription += self.sliceViewItemPattern.format(
                    viewName=viewName, orientation=orientation, color=color
                )
                actualViewNames.append(viewName)
                index += 1
                column += 1
    ...
    def volumeLightbox(
        self, volumeNode, layout=[3, 3], orientation="Axial", padRatio=0.1
    ):
        ...
        sliceNodesByViewName = self.viewerPerVolume(
            [
                volumeNode,
            ]
            * views,
        ...
        for viewName in sorted(sliceNodesByViewName.keys()):
            ...
            offset = lowBound + paddedRange * view / (views - 1.0)
    ...
    def create_linked_slice_views(
        ...
        for idx, vol in enumerate(volume_nodes, start=1):
            ...
            rgb = [
                int(round(v * 255)) for v in self.lookupTable.GetTableValue(idx)[:-1]
            ]
            color = f"#{rgb[0]:02X}{rgb[1]:02X}{rgb[2]:02X}"
            layout_desc += self.sliceViewItemPattern.format(
                viewName=view_name, orientation=orientation, color=color
            )
